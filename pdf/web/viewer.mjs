...
PDFPrintServiceFactory.initGlobals(PDFViewerApplication);
{
  ...
  var validateFileURL = function (file) {
    ...
    // TermLens modification: this viewer ships inside the extension and is
    // only reachable from the extension's own UI (never web-accessible), so
    // let it open any http(s) PDF — e.g. arxiv.org — via the extension's
    // host permissions. Non-http(s) schemes stay blocked.
    const fileProtocol = URL.parse(file, window.location)?.protocol;
    if (fileProtocol === "http:" || fileProtocol === "https:") {
      return;
    }
